...
sc_atac_trim_barcode = function(r1, r2, bc_file, output_folder = "", bc_start=-1, bc_length=-1, umi_start=0, umi_length=0, umi_in = "both") {
  ...
  if (!is.null(bc_file)) {
    ...
    if(file_ext(bc_file) != 'csv'){
      rcpp_sc_atac_trim_barcode_paired(output_folder, r1, bc_file,r2,write_gz)
    }
    else {
      ...
      rcpp_sc_atac_trim_barcode(output_folder, r1, r2, bc_file, bc_start, bc_length, umi_start, umi_length, umi_in, write_gz)
    }
  }else{
