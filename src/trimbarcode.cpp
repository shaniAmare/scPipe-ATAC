...
**/
    ...
    while (((l1 = kseq_read(seq1)) >= 0) && ((l2 = kseq_read(seq2)) >= 0))
    {
        ...
        /**
        char * str3 = (char *) malloc(2 + seq2_seqlen + seq1_namelen );
        strcpy(str3, seq2_seq);
        strcat(str3,"#");
        strcat(str3, seq1_name);
        
        seq1->name.s = str3;
        **/
        
        const int new_name_length = seq1_namelen + seq2_seqlen + 1;
        ...
        if (state == TWO_INDEX_WITH_UMI)
        {
            memcpy(seq1_name, seq2_seq + id1_st, id1_len * sizeof(char)); // copy index one
...
**/
        if (write_gz) {
            fq_gz_write(o_stream_gz, seq1, 0); // write to gzipped fastq file
        } else {
            fq_write(o_stream, seq1, 0); // write to fastq file
        }
       // free(str3);
        //break;
    }
